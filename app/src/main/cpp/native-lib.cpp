...
void init() {
    ...
    //2.打开输入视频文件
    if (int err_code = avformat_open_input(&pFormatCtx, inputPath, NULL, NULL) != 0) {
        LOGE("%s error code: %d ", "打开输入视频文件失败",err_code);
    }
    //3.获取视频信息
...
//开启播放
    ffmpegVideo->setFFmpegMusic(ffmpegMusic);
    ffmpegMusic->play();
    ffmpegVideo->play();
    isPlay = 1;
    //seekTo(0);
...
Java_com_mingyangzeng_ffmpgedemo_Play_play(JNIEnv *env, jobject instance, jstring inputPath_) {
...
extern "C"
JNIEXPORT void JNICALL
Java_com_mingyangzeng_ffmpgedemo_Play_display(JNIEnv *env, jobject instance, jobject surface) {
    //得到界面
    if (window) {
...
extern "C"
JNIEXPORT void JNICALL
Java_com_mingyangzeng_ffmpgedemo_Play_release(JNIEnv *env, jobject instance) {
    //释放资源
    if (isPlay) {
        isPlay = 0;
...
JNIEXPORT void JNICALL
Java_com_mingyangzeng_ffmpgedemo_Play_stop(JNIEnv *env, jobject instance) {
    //点击暂停按钮
    ffmpegMusic->pause();
    ffmpegVideo->pause();
...
Java_com_mingyangzeng_ffmpgedemo_Play_getTotalTime(JNIEnv *env, jobject instance) {
...
Java_com_mingyangzeng_ffmpgedemo_Play_getCurrentPosition(JNIEnv *env, jobject instance) {
    //获取音频播放时间
    return ffmpegMusic->clock;
}
extern "C"
JNIEXPORT void JNICALL
Java_com_mingyangzeng_ffmpgedemo_Play_seekTo(JNIEnv *env, jobject instance, jint msec) {
    seekTo(msec/1000);
}
extern "C"
JNIEXPORT void JNICALL
Java_com_mingyangzeng_ffmpgedemo_Play_stepBack(JNIEnv *env, jobject instance) {

}
extern "C"
JNIEXPORT void JNICALL
Java_com_mingyangzeng_ffmpgedemo_Play_stepUp(JNIEnv *env, jobject instance) {
    //点击快进按钮
    seekTo(5);
